...
const createTestConfig = async () => {
  ...
  console.log("Using chain:", process.env.LOCAL_CHAIN === "true" ? "foundry" : "baseSepolia");
};
...
async function main() {
  ...
  if (process.argv[2] == "setupPolicy") {
  ...
  } else if (process.argv[2] == "apply-policy") {
    // apply-policy - npx apply-policy <policyJSONFilePath> <address>
    const policyJSONFile = process.argv[3];
    if (!policyJSONFile) {
      console.error("Policy JSON file path is required.");
      return;
    }
    let policyData: string = fs.readFileSync(policyJSONFile, "utf8");
    if (!policyData) {
    ...
    const policyId = await setupPolicy(policyData);
    const callingContractAddress = getAddress(process.argv[4]);
    await applyPolicy(policyId, callingContractAddress);
  } else {
    ...
    console.log("     apply-policy <policyJSONFilePath> <address>");
  }
}
